...
class lstm_mlp(nn.Module):
    ...
    def __call__(self, X, deterministic):
        ...
        c1 = nn.LSTMCell.initialize_carry(
            jax.random.PRNGKey(0), (X.shape[0], ), 128)
        c1, X1 = self.lstm1(c1, X)
        c2 = nn.LSTMCell.initialize_carry(
            jax.random.PRNGKey(0), (X_flip.shape[0], ), 128)
        c2, X2 = self.lstm2(c2, X_flip)

        X = jnp.concatenate([X1, X2], axis=-1)

        X_flip = jnp.flip(X, axis=-1)
        c3 = nn.LSTMCell.initialize_carry(
            jax.random.PRNGKey(0), (X.shape[0], ), 256)
        c3, X1 = self.lstm3(c3, X)
        c4 = nn.LSTMCell.initialize_carry(
            jax.random.PRNGKey(0), (X_flip.shape[0], ), 256)
